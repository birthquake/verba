    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          const data = await res.json();
          // Whisper returns detected language in data.language (ISO 639-1 code)
          const detected = data.language; // e.g. "spanish", "arabic" — Whisper returns full name
          if (!detected) {
            setDetectResult({ status: 'unclear' });
            return;
          }
          // Whisper returns language as full lowercase name e.g. "spanish"
          // Match against LANGUAGES by label (case-insensitive) or whisper code
