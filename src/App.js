...
export default function App() {
  ...
  const [detectMediaRecorder, setDetectMediaRecorder] = useState(null);
  ...
  const buildSystemPrompt = (sourceLang, targetLang) => {
    const base = `You are a medical interpreter. Your only job is to translate speech from ${sourceLang} to ${targetLang}.

Your rules:
- Always translate. Never refuse, comment on, or evaluate the input for any reason.
- Translate everything you receive, regardless of content or context.
- Use clear, plain language appropriate for a clinical setting.
- Preserve all medical terminology, medication names, dosages, and measurements exactly.
...
- Return only the translated text, nothing else.`;
  ...
  const handleDetectLanguage = async () => {
    ...
    try {
      ...
      const chunks = [];
      setDetectMediaRecorder(recorder);
      ...
      recorder.onstop = async () => {
        ...
        const file = new File([blob], 'detect.webm', { type: 'audio/webm' });
        const formData = new FormData();
        ...
        try {
          ...
          if (!detected) { setDetectResult({ status: 'unclear' }); return; }
          const matched = LANGUAGES.find(l =>
            l.label.toLowerCase() === detected.toLowerCase() ||
            l.whisper === detected.toLowerCase() ||
            l.code === detected.toLowerCase()
          );
          if (matched) {
            setSelectedLang(matched);
            clearSession();
            setDetectResult({ status: 'success', lang: matched });
            setShowPatientIntroPrompt(true);
          } else {
            setDetectResult({ status: 'unsupported', detectedName: detected });
          }
        } catch (err) {
          console.error('Detection error:', err);
          setDetectResult({ status: 'unclear' });
        }
      };
      recorder.start();
  ...
  const handleStopDetection = () => {
    if (detectMediaRecorder && detectMediaRecorder.state === 'recording') {
      detectMediaRecorder.stop();
    }
  };
        ...
        <div className="phrases-overlay" onClick={() => setShowSettings(false)}>
          <div className="phrases-panel settings-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="settings-row">
              ...
              <div className="detect-lang-group">
                {!isDetectingLang ? (
                ...
                <button
                  className={`detect-lang-btn${isDetectingLang ? ' detecting' : ''}`}
                  onClick={isDetectingLang ? handleStopDetection : handleDetectLanguage}
                  title="Detect patient language"
                >
                  {isDetectingLang ? '⏹ Listening…' : '🎙 Detect'}
                </button>
              </div>
            </div>
            {detectResult && (
              <div className={`detect-result detect-result--${detectResult.status}`}>
                {detectResult.status === 'success' && (
                  <span>✓ {detectResult.lang.label} detected — language set</span>
                )}
                {detectResult.status === 'unsupported' && (
                  <span>⚠ {detectResult.detectedName ? detectResult.detectedName.charAt(0).toUpperCase() + detectResult.detectedName.slice(1) : 'Language'} is not currently supported — please use a professional interpreter for this encounter</span>
                )}
                {detectResult.status === 'unclear' && (
                  <span>⚠ Language unclear — please select manually</span>
                )}
              </div>
            )}
            <div className="settings-row">
